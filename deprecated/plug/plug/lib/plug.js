...
var Plug = stacks.Class({
  init: function(id){
    core.Asserted(valicore.isString(id),"first argument must be a string");
    this.contractId = id;
    this.outChannel = new channels.Channel();
    this.inChannel = new channels.SelectedChannel(this.contractId, MessagePicker);
    this.mutator = stacks.core.Middleware(funcs.bind(this.inChannel.emit,this.inChannel));
    this.mutator.add(function(d, next, end) {
        if (d.tag && d.tag != uuicore.tasks()) return null;
        return next(d);
    });
  },
  ...
  dispatch: function (t) {
    if(!(valicore.isInstanceOf(t,messages.MessagePack))) return;
...
var Plate = core.Class({
  init: function(id) {
      this.messages = new channels.Channel();
      this.mutator = stacks.core.Middleware(funcs.bind(this.messages.emit,this.messages));
  },
  plug: function(id){
    var pl =  new Plug(id);
    pl.bindPlate(this);
  ...
  createMessage: function(id){
     var self = this,
     mesg = new messages.MessagePack(id,uuicore.tasks());
  ...
  dispatchMessage:  function (id,uuid,data) {
    core.Asserted(valicore.exists(id),"one arguments is required (id)");
    var self = this, mesg = this.createMessage(id);
    if(valicore.exists(data) && valicore.exists(uuid)) mesg.pack({'uuid': uuid, data: data});
    mesg.ready(function(){
      self.dispatch(mesg);
    });
    return mesg.shell();
...
var PlugQueue = stacks.Class({
  init: function(pl){
    core.Asserted(valicore.isInstanceOf(pl,Plate),"argument must be an instance of plate");
    this.plate = pl;
    this.typeList = {};
    this.wq = stacks.core.WorkQueue();
    this.active = stacks.core.Switch();
  ...
  __pack: function(){
    ...
    for(m in this.typeList){
      e = this.typeList[m];
      if(stacks.Valicore.notExists(e)) return;
