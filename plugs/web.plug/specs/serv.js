...
stacks.Jazz('plate specification', function (_){
  ...
  var webr = web.ioBasic('we');
  webr.use(web.Plug('web.resource','http.server.request'),'webr');

  webr.Task.make('web.resource.new',{ model: 'admins', conf:{
    has: 'comments', 'params': {
      'id':'digits'
    }
  }});

  webr.Task.make('web.resource.update',{ model: 'admins', map:{
  ...
  _('can i send webrer config as tasks?',function($){
    ...
    $.for(webr.Task.make('io.server',{ port: 3000, address: '127.0.0.1'}));
  });
